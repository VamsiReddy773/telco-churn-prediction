"""
Serving app for the Telco Customer Churn model.

Exposes a FastAPI REST endpoint (/predict) for programmatic access and a
Gradio UI (/ui) for manual testing, both backed by the same inference logic.
"""
...
# Required for AWS Application Load Balancer health checks
@app.get("/")
def root():
    """Health check used by the load balancer / for quick sanity checks."""
    return {"status": "ok"}

class CustomerData(BaseModel):
    """The 18 features required for a churn prediction."""
    # Demographics
...
def get_prediction(data: CustomerData):
    """Run the churn model on one customer and return the prediction."""
    try:
...
def gradio_interface(
    ...
    """Wire the Gradio form fields into the same predict() used by the API."""
    data = {
        "gender": gender,
        "Partner": Partner,
...
demo = gr.Interface(
    ...
    description=(
        ...
        "The model is an XGBoost classifier trained on historical telecom customer data. "
        "Month-to-month contracts with fiber optic internet and electronic-check payments "
        "tend to churn more."
    ),
    examples=[
...
# This must be the final line to properly integrate Gradio with FastAPI
